...
const ProductDetails = () => {
  ...
  const [image, setImage] = useState(product.productResources?.[0]?.id);
  ...
  useEffect(() => {
    ...
    setImage(product?.productResources?.[0]?.id);
      ...
      <div className="flex flex-col md:flex-row px-10">
        <div className="w-full lg:w-[50%] md:w-[40%]">
          ...
          <div className="flex flex-col md:flex-row mt-2">
            <div className="w-full md:w-[20%] justify-center h-40px md:h-105">
              {/* Stack Image */}
              <div className="flex flex-col justify-center h-full">
                {product?.productResources?.map((item, index) => (
                  <button
                    ...
                    className="rounded-lg w-fit p-2 mb-2 cursor-pointer"
                  >
                    <img
                      src={item?.url}
                      className="h-15 w-15 rounded-lg bg-cover bg-center hover:scale-105 hover:border"
                      alt={"sample-" + index}
                    />
                  </button>
                ))}
            ...
            <div className="w-full md:w-[80%] flex justify-center md:pt-0 pt-10">
              <img
                src={product?.productResources?.find((item) => item?.id === image)?.url}
                className="h-full w-full max-w-130 border-gray-300 rounded-lg cursor-pointer object-cover"
                alt={product?.name}
              />
            </div>
          </div>
        ...
        <div className="lg:w-[50%] md:w-[60%] px-10 mt-3">
          {/* Product Description */}
          ...
          <div>
            <SizeFilter onChange={(values)=>{setSelectedSize(values?.[0] ?? '') }} sizes={sizes} hideTitle multi={false} />
          </div>
          <div>
          ...
          <div className="flex py-4">
            <button onClick={addItemToCart} className="bg-black rounded-lg hover:bg-gray-700 cursor-pointer">
              <div className="flex h-10.5 rounded-lg w-37.5 px-2 items-center justify-center bg-black text-white hover:bg-gray-700">
                <svg
                  width="17"
                  height="16"
                  className=""
